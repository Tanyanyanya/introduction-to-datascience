...
class Lab2:
    def __init__(self):
        self.n = 10000  # Кількість точок вимірювань
        self.number = int(self.n)  # Округлення кількості точок
        VidAnomalii = 40  # Відсоток аномалій у даних
        self.number_of_anomalies = int((self.number * VidAnomalii) / 100)  # Кількість аномалій
        self.dm = 0  # Середнє значення для нормального розподілу
        self.dsig = 5  # Стандартне відхилення для нормального розподілу
        self.SecAnomV = np.zeros((self.number_of_anomalies))  # Індекси аномалій
        self.SSAV = np.zeros((self.number_of_anomalies))  # Значення аномалій
    ...
    def GenerateNormalDistribution(self):
        for i in range(self.number_of_anomalies):
            self.SecAnomV[i] = mt.ceil(np.random.randint(1, self.number))  # Генерація індексів для аномалій
        self.S = np.random.normal(self.dm, 3 * self.dsig, self.number)  # Генерація нормального розподілу
        mS = np.median(self.S)  # Медіана вибірки
        dS = np.var(self.S)  # Дисперсія вибірки
    ...
    def ComputeStatistics(self):
        mSV0 = np.median(self.SV0)  # Медіана сигналу без аномалій
        dSV0 = np.var(self.SV0)  # Дисперсія сигналу без аномалій
        scvSV0 = mt.sqrt(dSV0)  # Середнє квадратичне відхилення
