'''
Виконала Луговець Тетяна ІП-12
...
# Функція analyze_real_data:

# Приймає списки dates (дати) та real_inflation_values (реальні значення інфляції).
# Обчислює та виводить статистичні характеристики реальних даних: середнє, стандартне відхилення, медіану, мінімум, максимум і дисперсію.
# Будує графік реальних даних інфляції за допомогою matplotlib.
...
# Функція generate_and_analyze_synthetic_data:

# Приймає списки dates (дати) та real_inflation_values (реальні значення інфляції).
# Використовує метод numpy.polyfit для побудови поліноміальної моделі (тренду) для реальних даних інфляції ступеня degree (в нашому випадку 12).
# Генерує синтетичні дані інфляції, додаючи до тренду випадковий шум, створений функцією numpy.random.normal.
# Обчислює статистичні характеристики синтетичних даних: середнє, стандартне відхилення, медіану, мінімум, максимум і дисперсію.
# Повертає синтетичні трендові значення та синтетичні значення інфляції.
...
#Головний блок програми (if __name__ == '__main__'):

#Завантажує дані з файлу data.json, який був згенерований на попередньому етапі.
#Розпаковує роки (parsed_dates) та значення інфляції (parsed_inflation_values) для подальшого аналізу
#Виводить графіки 
